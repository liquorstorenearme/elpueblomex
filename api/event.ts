...
export default async function handler(req: Request): Promise<Response> {
  ...
  const fallbackTo = process.env.EP_EVENT_TO || process.env.EP_TO_EMAIL || "hello@elpueblomex.com";
  ...
  if (g("website")) return back("/event-space/", req.url, { sent: "1" });
  ...
  const partySize = g("party_size").slice(0, 10);
  const date = g("date").slice(0, 40);
  const time = g("time").slice(0, 40);
  ...
  <div style="background:#fff;padding:32px 28px;border-radius:12px;border:1px solid #d84a1e;">
    <h2 style="margin:0 0 4px;font-size:22px;">New event inquiry</h2>
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;font-size:14px;line-height:1.55;">
      ${row("Name", name)}${row("Email", email)}${row("Phone", phone)}${row("Organization", organization)}${row("Location", location)}${row("Party size", partySize)}${row("Date", date)}${row("Time", time)}${row("Comments", message, true)}
    </table>
  ...
  if (!r.ok) { console.error("Resend error:", r.status, await r.text()); return back("/event-space/", req.url, { err: "send" }); }
  return back("/event-space/", req.url, { sent: "1" });
}
