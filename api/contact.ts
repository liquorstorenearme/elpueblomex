import { looksLikeSpam } from "./_spam";
import { rateLimit, clientIp } from "./_ratelimit";

export const config = { runtime: "edge" };

const RL_WINDOW_S = 5 * 60;
const RL_MAX = 5;

const esc = (s: string) =>
  String(s ?? "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;");

function back(path: string, url: string, params: Record<string, string> = {}): Response {
  const u = new URL(path, url);
  for (const [k, v] of Object.entries(params)) u.searchParams.set(k, v);
  return Response.redirect(u.toString(), 302);
}

function row(label: string, value: string): string {
  if (!value) return "";
  return `<tr><td style="padding:8px 12px 8px 0;border-bottom:1px solid #eee;color:#4a362a;vertical-align:top;width:160px;">${esc(label)}</td><td style="padding:8px 0;border-bottom:1px solid #eee;">${esc(value)}</td></tr>`;
}

export default async function handler(req: Request): Promise<Response> {
  if (req.method !== "POST") return new Response("Method not allowed", { status: 405 });

  const resendKey = process.env.RESEND_API_KEY;
  const toEmail = process.env.EP_CONTACT_TO || process.env.EP_TO_EMAIL || "hello@elpueblomex.com";
  const toList = toEmail.split(",").map((s) => s.trim()).filter(Boolean);
  const fromEmail = process.env.EP_FROM_EMAIL || "noreply@elpueblomex.com";

  if (!resendKey) return back("/contact/", req.url, { err: "config" });

  const ip = clientIp(req);
  if ((await rateLimit("form:contact", ip, RL_MAX, RL_WINDOW_S)).limited) return back("/contact/", req.url, { err: "rate" });

  let form: FormData;
  try { form = await req.formData(); } catch { return back("/contact/", req.url, { err: "parse" }); }
  const g = (k: string) => String(form.get(k) || "").trim();

  if (g("website")) return back("/contact/", req.url, { sent: "1" }); // honeypot

  const name = g("name").slice(0, 100);
  const email = g("email").slice(0, 150);
  const location = g("location").slice(0, 60);
  const message = g("message").slice(0, 5000);

  if (!name || !email || !message) return back("/contact/", req.url, { err: "missing" });
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) return back("/contact/", req.url, { err: "email" });
  if (looksLikeSpam(message, email, name)) return back("/contact/", req.url, { sent: "1" });

  const html = `<!DOCTYPE html><html><body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#1b1008;max-width:720px;margin:0 auto;padding:24px;background:#faf0d8;">
  <div style="background:#fff;padding:32px 28px;border-radius:12px;border:1px solid #d84a1e;">
    <h2 style="margin:0 0 4px;font-size:22px;">New contact message</h2>
    <p style="margin:0 0 24px;color:#4a362a;font-size:13px;letter-spacing:0.08em;text-transform:uppercase;font-weight:600;">El Pueblo Mexican Food · elpueblomex.com</p>
    <table cellpadding="0" cellspacing="0" style="width:100%;border-collapse:collapse;font-size:14px;line-height:1.55;">
      ${row("Name", name)}${row("Email", email)}${row("Location", location)}
    </table>
    <h3 style="font-size:12px;letter-spacing:0.14em;text-transform:uppercase;color:#d84a1e;margin:24px 0 8px;">Message</h3>
    <div style="white-space:pre-wrap;font-size:14px;line-height:1.55;">${esc(message)}</div>
    <p style="margin-top:24px;color:#6a5a4a;font-size:11px;border-top:1px solid #eee;padding-top:16px;">IP: ${esc(ip)} · ${new Date().toISOString()}</p>
  </div></body></html>`;

  const r = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { Authorization: `Bearer ${resendKey}`, "Content-Type": "application/json" },
    body: JSON.stringify({
      from: `El Pueblo Contact <${fromEmail}>`,
      to: toList,
      reply_to: email,
      subject: `Contact — ${name}${location ? ` — ${location}` : ""}`,
      html,
    }),
  });

  if (!r.ok) { console.error("Resend error:", r.status, await r.text()); return back("/contact/", req.url, { err: "send" }); }
  return back("/contact/", req.url, { sent: "1" });
}
