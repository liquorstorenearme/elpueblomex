...
export default async function handler(req: Request): Promise<Response> {
  ...
  const toEmail = process.env.EP_CATERING_TO || process.env.EP_TO_EMAIL || "hello@elpueblomex.com";
  ...
  if (looksLikeSpam(message, email, name)) return back("/catering/", req.url, { sent: "1" });
